...
test('chat uses Responses API and returns text with mocked provider',async()=>{
  ...
  global.fetch=async(url,opts)=>{assert.match(String(url),/\/v1\/responses$/);const body=JSON.parse(opts.body);assert.ok(body.max_output_tokens<=700);return new Response(JSON.stringify({output_text:'Короткая подсказка'}),{status:200,headers:{'content-type':'application/json'}});};
